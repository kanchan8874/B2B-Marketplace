...
import { getPendingBuyerKYCs, approveBuyerKYC, rejectBuyerKYC } from '../../services/kycService.js'

const ITEMS_PER_PAGE = 10

const BuyerKYCManagement = () => {
  ...
  const loadKYCs = async () => {
    try {
      setLoading(true)
      const response = await getPendingBuyerKYCs(statusFilter, page, ITEMS_PER_PAGE)
      if (response.data) {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (actionType === 'approve') {
        await approveBuyerKYC(selectedKYC._id)
        alert('KYC approved successfully')
      } else if (actionType === 'reject') {
        await rejectBuyerKYC(selectedKYC._id, rejectionReason)
        alert('KYC rejected')
      }
      setShowModal(false)
      setSelectedKYC(null)
  ...
  const filteredKycs = searchQuery
    ? kycs.filter(
        (kyc) =>
          ...
          kyc.buyer?.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
          kyc.buyer?.email?.toLowerCase().includes(searchQuery.toLowerCase()),
      )
    : kycs
  ...
  const columns = [
    {
      ...
      cell: (row) => (
        <div>
          ...
          <p className="text-xs text-neutral-500">{row.buyer?.email}</p>
        </div>
      ),
    },
    {
      ...
      cell: (row) => (
        <div>
          <p className="text-sm text-neutral-900">
            {row.city && row.state ? `${row.city}, ${row.state}` : 'Not provided'}
          </p>
        </div>
      ),
    },
      ...
      <div className="rounded-3xl border border-blue-100 bg-gradient-to-r from-blue-50/80 via-white to-emerald-50/80 px-5 py-4 shadow-[0_18px_45px_rgba(15,23,42,0.06)]">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold text-neutral-900 leading-snug">Buyer KYC Management</h1>
            <p className="mt-1 text-sm text-neutral-600">
              Review and approve buyer verification submissions to keep the marketplace safe and trusted.
            </p>
          </div>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
          <div className="flex-1">
            <FormField
              ...
              name="search"
              label="Search buyers"
              placeholder="Search by business name, buyer email..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              icon={Search}
            />
            <p className="mt-1 text-[11px] text-neutral-500">
              Use search + status filters to quickly locate a specific buyer application.
            </p>
          </div>
          <div className="flex flex-wrap gap-2" role="tablist" aria-label="Filter buyer KYC by status">
      ...
      <Card className="border-blue-100 bg-white shadow-[0_16px_40px_rgba(15,23,42,0.06)]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-14 text-center">
            ...
            <p className="text-sm font-semibold text-neutral-900">No KYC submissions found</p>
            <p className="mt-1 max-w-md text-xs text-neutral-600">
              Once buyers start submitting their verification details, they will appear here for your review and
              approval.
            </p>
          </div>
        ) : (
        ...
        >
          <div className="space-y-6">
            ...
            {actionType === 'approve' && (
              <div className="space-y-4">
                <p className="text-sm text-neutral-700">
                  Are you sure you want to approve this KYC submission? The buyer will receive a verified badge.
                </p>
            ...
            {actionType === 'reject' && (
              <div className="space-y-4">
                <p className="text-sm text-neutral-700">
                  Please provide a reason for rejecting this KYC submission. The buyer will be notified.
                </p>
                <FormField
                  id="rejectionReason"
...
export default BuyerKYCManagement
